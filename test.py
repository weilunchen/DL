...
try:
	...
	class UNet(nn.Module):
		def __init__(self, in_channels=3, out_channels=1):
			...
			self.test1 = nn.Conv2d(1, 64, 3, stride = 1, padding = 1)

			self.test2 = nn.ConvTranspose2d(64, 1, 3, stride = 1, padding = 1)

			self.test_relu = nn.ReLU(inplace=True)

			self.test_batch = nn.BatchNorm2d(64)
		...
		def forward(self, x):
			...
			# skip_connection = self.skip_convs[0](x)
			# x_skip = skip_connection

			# concat_skip_connection = torch.cat((x_skip, x_skip), dim=1)
			# x = self.ups[-1](concat_skip_connection)
	...
	def train(model, train_loader, test_loader, epochs, criterion, optimizer, schedular):
		...
		for epoch in range(epochs):
			...
			for data, target in train_loader:
				...
				# output = torch.clamp(torch.round(output.squeeze(1)), min=0, max=1)
	...
	if __name__ == "__main__":
		...
		schedular = lr_scheduler.MultiplicativeLR(optimizer, lr_lambda = lambda epoch: 0.9999)
